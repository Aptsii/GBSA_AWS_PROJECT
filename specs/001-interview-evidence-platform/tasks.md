...
- [X] T172 Integration — Replace Lane A campaign/consent fakes for Lane B and add real boundary tests in `backend/tests/integration/cross_module/test_a_to_b.py` (FR-015, FR-021, QG-01, QG-04)
- [X] T173 Integration — Replace Lane B strategy/retrieval fakes for Lane C and add real boundary tests in `backend/tests/integration/cross_module/test_b_to_c.py` (FR-028, FR-030, QG-01, QG-10)
- [X] T174 Integration — Replace Lane C Turn/media fakes for Lane D and add real boundary tests in `backend/tests/integration/cross_module/test_c_to_d.py` (FR-037-FR-041, QG-01, QG-02)
- [X] T175 Integration — Connect Lane D report/deletion projections to Lane A company views in `backend/tests/integration/cross_module/test_d_to_a.py` (FR-042-FR-047, QG-01, QG-03, QG-05)
- [X] T176 Integration — Wire all router fragments and worker handlers in `backend/src/interview_evidence/main.py` (FR-050, QG-01, QG-11)
- [X] T177 Integration — Wire all feature routes without changing lane feature internals in `apps/company-console/src/app/featureRoutes.ts` and `apps/applicant-interview/src/app/featureRoutes.ts` (QG-01)
...
- [X] T190 Integration — Run and record every command/outcome from `specs/001-interview-evidence-platform/quickstart.md` (QG-01-QG-16)
...
- [X] T192 Integration — Run `$speckit-converge` and append every remaining spec/plan/code gap to `specs/001-interview-evidence-platform/tasks.md` (QG-01-QG-16)
...
- [X] T193 CRITICAL Integration — Compose production `ApplicationRuntimes` from configured SQLAlchemy repositories and AWS adapters, mount every `/v1` router in the API container factory, and prove the deployed OpenAPI exposes tenant- and consent-guarded feature routes per FR-001-FR-052 and QG-01/QG-11 (missing)
- [X] T194 CRITICAL Integration — Implement the tenant-scoped versioned queue receive, dispatch, retry, acknowledgement and shutdown loop for the registered worker handlers in `backend/src/interview_evidence/workers/__main__.py` per FR-050 and plan worker rules (missing)
- [X] T195 [P] Integration — Mount the company and applicant feature registries into real React Router navigation and render the owned lane screens instead of static application shells per T177 and QG-01 (partial)
- [X] T196 Integration — Replace the snapshot-only thin journey with a real composed company-to-human-decision flow over the production runtime, PostgreSQL and local AWS-compatible services per T179 and QG-01 (partial)
- [X] T197 [P] Lane A — Compose independent `foundation`, `data-ai` and `application` roots for stage and prod with the required network, edge, identity, data, async, AI/search, observability and compute modules per plan environment roots and QG-13/QG-16 (contradicts)
- [X] T198 Integration — Align deployment root selection with all environment state roots, register ECS task definitions with the requested immutable `image_digest`, and wait for migration task success before service rollout per T170 and QG-13/QG-16 (partial)
- [X] T199 Integration — Extend the enabled stage smoke into an authenticated CloudFront-to-API business journey that reaches the configured AWS data and AI services, rather than independent health/resource checks, per T189 and QG-13 (partial)
- [X] T200 [P] Lane C — Rework pilot load validation to use a shared production-like runtime, transition all five concurrent interviews to `COMPLETED`, and assert cross-company data separation across hundreds of accumulated records per SC-007/SC-013 (partial)
- [X] T201 [P] Lane D — Measure Evidence selection through actual browser media playback start, including signed playback retrieval and the two-second threshold, per SC-012 (partial)
- [X] T202 Integration — Add versioned production metrics and tests for stage latency, retries, reconciliation lag, queue age and degraded-mode use at API and worker boundaries per FR-051, plan operational design and Constitution V (missing)
- [X] T203 Integration — Correct `validation-report.md` to show the convergence findings as unresolved until T193-T202 pass, then rerun every affected FR/SC/QG gate and record the final result per T191 (contradicts)

## Phase 9: Convergence

- [X] T204 CRITICAL Lane D — Scope SQLAlchemy human-review idempotency replay and database uniqueness by `company_id`, with a migration and cross-company same-key regression test, per FR-005, SC-014 and Constitution II (contradicts)
- [X] T205 Integration — Align the versioned EMF metric namespace and queue-age unit with the deployed Terraform alarm contract, and add a cross-layer contract test per FR-051 and plan operational design (contradicts)

## Phase 10: Convergence

- [X] T206 CRITICAL Integration — Add a shared generated-contract-aware browser transport with environment configuration, company bearer and applicant cookie session handling, idempotency headers, safe error parsing and request cancellation under `packages/web-client/`, then consume it from both SPA shells per US1-US4, FR-005, FR-032 and QG-01/QG-08 (missing)
- [X] T207 CRITICAL Lane A — Replace the company hiring prototype state with real `/v1/me`, position, competency-model, campaign, publish and invitation API mutations and queries in `apps/company-console/src/features/hiring/`, including loading, validation, retry and tenant-safe failure tests per US1, FR-006-FR-012 and SC-001 (partial)
- [X] T208 CRITICAL Lane A — Connect the applicant access journey to invitation exchange, identity verification and consent APIs with invitation-scoped cookie/session persistence and consent-gate failure tests in `apps/applicant-interview/src/features/access/` per US2/AC1-AC2 and FR-013-FR-015 (partial)
- [X] T209 HIGH Lane B — Connect submission upload intents, direct object upload, submission creation and analysis-status polling to the real API in `apps/applicant-interview/src/features/submissions/`, preserving partial-analysis and retry states per US2, FR-016-FR-022 and QG-09/QG-10 (partial)
- [X] T210 CRITICAL Lane C — Connect equipment checks, interview session start/resume, versioned WebSocket messages, recording chunk uploads, transcript/question events, degraded modes and checkpoint recovery in `apps/applicant-interview/src/features/interview/` per US3, FR-023-FR-036 and QG-08 (partial)
- [X] T211 CRITICAL Lane D — Connect report, timeline, signed playback, assessment override, note/bookmark, human-only final decision and deletion request/status APIs in `apps/company-console/src/features/review/` per US4, FR-037-FR-048 and SC-012/SC-015 (partial)
- [X] T212 CRITICAL Integration — Add local API reachability for both SPA origins, production-safe runtime configuration, and a browser-level company-to-human-decision integration journey against the Compose API in `backend/src/interview_evidence/main.py`, `compose.yaml`, `apps/Containerfile` and `tests/e2e/` per FR-001-FR-052 and QG-01/QG-11 (missing)

## Phase 11: Real Browser Runtime Closure

- [X] T213 CRITICAL Integration — Extend the shared object-storage port and S3 adapter with tenant-scoped signed PUT creation and post-upload digest/size verification, including deterministic fake and adapter tests under `backend/src/interview_evidence/shared/aws_clients/` and `backend/tests/unit/shared/` per FR-016, FR-027 and QG-08 (missing)
- [X] T214 CRITICAL Lane B — Replace placeholder submission upload URLs with the shared signed PUT contract and verify the uploaded object before submission registration under `backend/src/interview_evidence/submission_analysis/`, including applicant scope, expiry, digest and replay tests per FR-016-FR-019 and QG-08/QG-10 (missing)
- [X] T215 CRITICAL Lane C — Replace placeholder recording upload URLs with the shared signed PUT contract and verify uploaded chunks into contiguous durable recording state under `backend/src/interview_evidence/interview_engine/`, including reconnect sequence tests per FR-027-FR-029 and QG-08 (missing)
- [X] T216 CRITICAL Lane C — Replace the ACK-only SQLAlchemy WebSocket handler with versioned session start, question, transcript/finalize, checkpoint, resume, degraded and completion orchestration using only Lane C repositories and public contracts per FR-023-FR-036 and QG-08 (missing)
- [X] T217 CRITICAL Integration — Add a safe local company principal/bootstrap fixture and executable Compose browser journey configuration without weakening stage/prod authentication under `backend/src/interview_evidence/main.py`, `compose.yaml` and `tests/e2e/` per FR-001-FR-052 and QG-01/QG-11 (missing)
...
- [X] T219 CRITICAL Lane C + Integration — Stream browser audio or explicit text-only applicant answers through the versioned WebSocket, wire a configured transcription adapter into the production runtime, consume the persisted strategy and criterion axis for questions, and complete/reconnect without fabricated Evidence per US3/AC1-AC6 and FR-024-FR-036 (partial)
- [X] T220 CRITICAL Lane D + Integration — Emit media post-processing and report-generation events when an interview completes, execute the registered workers, persist transcript/media/report/Evidence projections, and make report and timeline APIs load the completed session per US4/AC1-AC2/AC5 and FR-037-FR-042 (missing)
- [X] T221 HIGH Lane A + Integration — Replace production-facing raw bearer and invitation-token entry with company identity login/session handling and automatic expiring invitation deep-link exchange while retaining isolated local fixtures per US1/AC2, US2/AC1 and FR-006/FR-011/FR-013 (partial)
- [ ] T222 CRITICAL Integration — Extend the Compose Chromium journey through submission analysis, strategy readiness, equipment check, recording/transcription, reconnect, interview completion, report Evidence seek and explicit human decision, and restore every QG-01/QG-03/QG-11 gate including the human-control static check per plan browser/E2E strategy and QG-01/QG-03/QG-11 (partial)
